...
@EnableHotRodHttpSession(config = @HotRod(uri = "${infinispan.server.uri}", configuration = "${infinispan.server.configuration}"), manager = @SessionManager(marshaller = SessionAttributeMarshaller.PROTOSTREAM, granularity = SessionPersistenceGranularity.ATTRIBUTE))

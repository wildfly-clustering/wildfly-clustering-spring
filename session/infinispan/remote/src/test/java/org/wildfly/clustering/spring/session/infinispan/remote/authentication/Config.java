...
@EnableHotRodIndexedHttpSession(config = @HotRod(uri = "${infinispan.server.uri}", configuration = "${infinispan.server.configuration}"), manager = @SessionManager(marshaller = SessionAttributeMarshaller.JBOSS, granularity = SessionPersistenceGranularity.ATTRIBUTE), indexing = @Indexing)
